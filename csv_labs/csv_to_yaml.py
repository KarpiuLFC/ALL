...
with open(source, 'r') as file: #open file with read mode
    rows = csv.reader(file) # read rows from file
    headers = next(rows) # avoid headers
    for line in file.readlines(): #read file line by line
        line = line.strip() #read only content without unnecessary characters       
        (type, action, view, record, value, alias, new_record, new_value) = line.split(";") #define number and names of column from file to script, only number of columns must be equal 
